...
class CloudDBDatabase extends Dexie {
  ...
  constructor() {
    ...
    this.version(2).stores({
      metrics: "++id, instanceId, timestamp, [instanceId+timestamp]",
    });
  }
}
...
export async function seedDatabaseIfEmpty() {
  ...
  console.log(
    "IndexedDB: Local browser environment database successfully seeded!",
  );
}
...
export async function runStorageGarbageCollection(
  maxRecordsPerInstance: number = 100,
) {
  try {
    const instancesList = await db.instances.toArray();

    for (const instance of instancesList) {
      // 1. Usamos o índice composto para contar rápido
      const instanceRecordsCount = await db.metrics
        .where("[instanceId+timestamp]")
        .between([instance.id, 0], [instance.id, Infinity])
        .count();

      if (instanceRecordsCount <= maxRecordsPerInstance) continue;

      const recordsToDeleteCount = instanceRecordsCount - maxRecordsPerInstance;

      // 2. Usamos o MESMO índice composto para pegar as chaves mais antigas
      // Como o índice composto já vem ordenado por timestamp nativamente,
      // apanha logo os primeiros (mais antigos) daquela instância!
      ...
      if (legacyKeysCollection.length > 0) {
        ...
        console.log(
          `[Storage GC]: Safely purged ${legacyKeysCollection.length} stale snapshots for cluster ${instance.id}.`,
        );
      }
    }
  } catch (error) {
    console.error(
      "Storage Garbage Collection loop encountered an exception:",
      error,
    );
  }
}
